...
# ==================== 消融组: MBD + MLP（去Fine Gaussian引导） ====================
# 验证问题: Fine Gaussian的空间引导机制贡献了多少？
model = MBDCompressor3D(
    ...
    basis_kernel_scale=0.20,    # 初始尺度（与HI-NE-GBD完整模型对齐）
    mlp_hidden=128     # MLP隐藏层大小（与HI-NE-GBD完整模型对齐）
)
...
plt.suptitle('MBD + 3D Gaussians + MLP Decoder - Ablation: No Fine Gaussian', fontsize=16, y=1.02)
plt.tight_layout()
plt.show()

print("\nDemo completed!")
print("="*70)
print("消融组: MBD + MLP（去Fine Gaussian引导）演示完成！")
print("="*70)
print(f"实验配置:")
