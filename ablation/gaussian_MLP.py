...
# ==================== Siggraph Baseline: 3D高斯+MLP（去MBD） ====================
# 验证问题: MBD框架相比纯Gaussian+MLP范式的优势有多大？
num_gaussians = 256  # 高斯数量（与gaussian_control统一）
model = GaussianProbeCompressor3D(
    num_gaussians=num_gaussians,
    latent_dim=8,     # 潜在特征维度
    sh_dim=3,         # RGB 3通道（与其他对照组统一）
    mlp_hidden=128     # MLP隐藏层
)
...
# 训练模型（统一训练参数：总epochs=2500, batch_size=4096，与消融组对齐）
solver = GaussianCompressionSolver3D(model, lambda_reg=1e-5)  # 与其他消融组统一正则化强度
losses = solver.train(probe_positions, probe_SH_data, epochs_main=2500, epochs_quant_finetune=0, batch_size=4096)
...
def compute_ssim(img1, img2, window_size=11):
    """Compute SSIM for multi-channel images (windowed version, aligned with other ablation groups)"""
...
# 对每个通道计算PSNR和SSIM（使用Z中间切片，与其他消融组统一）
z_slice = D // 2
psnr_values = []
ssim_values = []
for c in range(C):
    gt_ch = ground_truth_SH[z_slice, :, :, c].numpy()
    rec_ch = reconstructed_vol[z_slice, :, :, c]
    psnr_c = compute_psnr(gt_ch, rec_ch)
    ssim_c = compute_ssim(gt_ch, rec_ch)
...
plt.suptitle('3D Gaussian + MLP Compression - Siggraph Baseline (No MBD)', fontsize=16, y=1.02)
plt.tight_layout()
#plt.savefig('output/3Dgaussian+MLP_1400+100epochs.png', dpi=150, bbox_inches='tight')
plt.show()

print("\n" + "="*70)
print("Siggraph Baseline: 3D高斯+MLP压缩演示完成！")
print("="*70)
print(f"实验配置:")
