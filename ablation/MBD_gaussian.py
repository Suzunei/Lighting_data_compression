...
class MBDCompressor3D(nn.Module):
    ...
    def forward(self, coords):
        ...
        # 4. Reconstruct signal with learnable scale: f̂(x) = Σ_l scale_l * c_l(x) * b_l(x)
        mbd_scale = torch.exp(self.mbd_log_scale)  # [L] - 可学习的scale因子
        scaled_coeff = moving_coeff * mbd_scale.unsqueeze(0)  # [Q, L] * [1, L] = [Q, L]
        reconstruction = torch.sum(scaled_coeff.unsqueeze(-1) * moving_basis, dim=1)  # [Q, D]
    ...
    def get_compression_ratio(self, original_size):
        ...
        compressed_size = (coeff_params + basis_params + mbd_scale_params) * 4
...
# ==================== 消融组: 纯MBD（去Fine Branch） ====================
# 验证问题: Fine Branch (Fine Gaussian + MLP + PE) 贡献了多少？
model = MBDCompressor3D(
    num_bases=8,      # 基的数量L（与HI-NE-GBD完整模型对齐）
    coeff_res=128,      # 系数3D高斯数量M（与HI-NE-GBD完整模型对齐）
    basis_res=128,      # 基3D高斯数量N（与HI-NE-GBD完整模型对齐）
    data_dim=C,       # 数据维度D (RGB)
    coeff_kernel_scale=0.15,   # 初始尺度（与HI-NE-GBD完整模型对齐）
    basis_kernel_scale=0.2     # 初始尺度（与HI-NE-GBD完整模型对齐）
)
...
losses = solver.train(coords, target_data, epochs=2500, batch_size=4096)
...
plt.suptitle('MBD + 3D Gaussians - Ablation: No Fine Branch', fontsize=16, y=1.02)
plt.tight_layout()
plt.show()

print("\nDemo completed!")
print("="*70)
print("消融组: 纯MBD（去Fine Branch）演示完成！")
print("="*70)
print(f"实验配置:")
print(f"  - 模型类型: MBD + 3D Trainable Gaussians (去Fine Branch)")
